...
첨부된 스토리보드를 옮긴 **세로 1080×1920 / 30fps / 1분 08초 · 한국어 내레이션 포함** 숏츠 영상 렌더러입니다.
...
| 시작 | 씬 | 내용 |
|---|---|---|
| 0:00 | intro | **4분할 오프닝** — 프로/일반인 × 톱/임팩트 네 칸이 뜬 뒤 2패널로 합쳐진다 |
| 0:04 | joints | 어깨·팔꿈치·골반·무릎·발목 대응 표시 |
| 0:09 | rotation | 회전량 비교 (어깨 92°/76°, 골반 45°/28°) |
| 0:18 | sequence | 2패널 + **필름스트립 4컷** — 골반→몸통→팔→클럽 순서대로 점등 |
| 0:26 | amateur | 팔·클럽이 먼저 나가는 동작 (같은 시점 프로 동작을 고스트로 겹침) |
| 0:31 | chain | 에너지 전달 순서와 헤드 스피드 |
| 0:36 | path | 클럽 헤드 궤적 비교 |
| 0:40 | impact | 임팩트 몸통 열림 25° / 8° |
| 0:46 | grid | **6분할 멀티 화면** — 톱·임팩트·피니시 × 프로·일반인 |
| 0:51 | statement | "효율적으로 연결하느냐의 차이" |
| 0:55 | summary | 핵심 정리 4줄 |
| 1:00 | next | 다음 편 예고 |
| 1:05 | outro | 좋아요 · 구독 · 알림 |

멀티 화면은 세 종류입니다. 오프닝의 4분할→2패널 모프는 `absPanel()` + `lerpRect()` 로
칸의 사각형을 보간해 만들고, 필름스트립(`.filmcell`)과 6분할(`.gridcell`)은 같은 스윙 모델을
서로 다른 시점 `t` 로 여러 번 그린 것입니다. 씬마다 아주 느린 줌인(1.8%)도 함께 걸려 있습니다.
